...
# ./task.c:27:         printf("\nNumber of N: %d", N_count);
	mov	eax, r13d	# tmp111, N_count
	mov	esi, eax	#, tmp111
	lea	rdi, .LC1[rip]	#,
	mov	eax, 0	#,
	call	printf@PLT	#
# ./task.c:28:         printf("\nNumber of Z: %d", Z_count);
	mov	eax, r14d	# tmp112, Z_count
	mov	esi, eax	#, tmp112
	lea	rdi, .LC2[rip]	#,
	mov	eax, 0	#,
	call	printf@PLT	#
# ./task.c:29:         printf("\nNumber of X: %d", X_count);
	mov	eax, r15d	# tmp113, X_count
	mov	esi, eax	#, tmp113
	lea	rdi, .LC3[rip]	#,
	mov	eax, 0	#,
	call	printf@PLT	#
...
# ./task.c:76:         string[j] = (char) (64 + rand() % 119);
	mov	edx, eax	# _7, _6
# ./task.c:76:         string[j] = (char) (64 + rand() % 119);
	mov	eax, r12d	# tmp96, j
	mov	BYTE PTR -144[rbp+rax], dl	# string, _7
# ./task.c:75:     for (int j = 0; j < length; ++j) {
	add	r12d, 1	# j,
.L21:
# ./task.c:75:     for (int j = 0; j < length; ++j) {
	mov	eax, r12d	# tmp97, j
